...
export default function Customers() {
  ...
  return (
    <div className="space-y-6 w-full max-w-7xl mx-auto">
      {/* ── Header ──────────────────────────────────────────────── */}
      <div>
        <h1 className="text-3xl font-bold text-ink-primary tracking-tight">Customer Profiling</h1>
        <p className="text-sm text-ink-secondary mt-1">
          Understand who's buying, who's slipping away, and who deserves the red carpet
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="card p-5 lg:col-span-1">
          <div className="flex items-baseline justify-between mb-4">
            <h2 className="text-base font-semibold text-ink-primary">Segments</h2>
            <span className="text-xs text-ink-tertiary">RFM-based</span>
          </div>
              ...
              return (
                <button
                  ...
                  className={clsx(
                    ...
                    segmentFilter === key
                      ? 'bg-surface-overlay border-line-strong'
                ...
                >
                  ...
                  <span className="text-sm font-semibold text-ink-primary tabular-nums">{count}</span>
                </button>
              )
            })}
          </div>
        ...
        <div className="card p-5 lg:col-span-2">
          <div className="flex items-baseline justify-between mb-4">
            <div>
              <h2 className="text-base font-semibold text-ink-primary">Average Order Value</h2>
              <p className="text-xs text-ink-tertiary mt-0.5">Last 6 months</p>
            </div>
            <span className="badge badge-primary">KES {formatKES(overview.kpis.avg_aov)}</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="card p-5">
          <h2 className="text-base font-semibold text-ink-primary mb-4">Top Spenders</h2>
          <div className="space-y-3">
            {overview.topSpenders.map((c, i) => (
              <button
                key={c.id}
                onClick={() => navigate(`/customers/${c.id}`)}
                className="w-full flex items-center gap-3 text-left hover:bg-surface-overlay p-2 -mx-2 rounded-lg transition-colors"
              >
                <span className="text-xs text-ink-tertiary w-4 tabular-nums">{i + 1}</span>
                <Avatar customer={c} size={32} />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-ink-primary truncate">{c.name}</p>
                  <p className="text-xs text-ink-tertiary">{c.total_orders} orders</p>
                </div>
        ...
        {/* Most frequent */}
        <div className="card p-5">
          <h2 className="text-base font-semibold text-ink-primary mb-4">Most Frequent Buyers</h2>
          <div className="space-y-3">
            {overview.topFrequent.map((c, i) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-ink-primary truncate">{c.name}</p>
                  <p className="text-xs text-ink-tertiary">KES {formatKES(c.total_spent)}</p>
                </div>
                <p className="text-sm font-semibold text-ink-primary tabular-nums">
        ...
        {/* Channel breakdown */}
        <div className="card p-5">
          <h2 className="text-base font-semibold text-ink-primary mb-4">Acquisition Channel</h2>
            ...
            {overview.channelBreakdown.map((ch, i) => (
              <div key={ch.name} className="flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className="text-ink-secondary capitalize">{ch.name}</span>
                </div>
                <span className="text-ink-primary tabular-nums">{ch.percent}%</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="card p-5">
        <h2 className="text-base font-semibold text-ink-primary mb-4">Top Products Purchased</h2>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-lg font-semibold text-ink-primary">All Customers</h2>
            <p className="text-xs text-ink-tertiary mt-0.5">{filtered.length} of {customers.length} shown</p>
          </div>
          <div className="flex items-center gap-2">
...
function KpiCard({ icon: Icon, label, value, sub }) {
  return (
    <div className="card p-4">
      ...
      <p className="text-2xl font-semibold text-ink-primary tabular-nums tracking-tight">{value}</p>
...
function CustomerRow({ customer, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center gap-4 p-4 hover:bg-surface-overlay transition-colors text-left"
    >
      <Avatar customer={customer} />
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-ink-primary truncate">{customer.name}</p>
        <p className="text-xs text-ink-tertiary truncate">{customer.email} · {customer.location}</p>
      </div>
      <div className="hidden md:flex items-center gap-2">
        <SegmentBadge color={meta.color}>
          <Icon size={12} strokeWidth={2.5} />
        </SegmentBadge>
        <span className="text-xs text-ink-secondary w-16">{meta.label}</span>
      </div>
      <div className="hidden sm:block text-right">
        <p className="text-sm font-semibold text-ink-primary tabular-nums">
          KES {formatKES(customer.total_spent)}
        </p>
        <p className="text-xs text-ink-tertiary">{customer.total_orders} orders · {timeAgo(customer.last_order_date)}</p>
      </div>
