...
// A few starter prompts to show when the chat is empty.
const SUGGESTIONS = [
...
function DataTable({ rows }) {
  ...
  // Column order: prefer a sensible layout, fall back to object keys.
...
export default function CustomerAIChat() {
  const [messages, setMessages] = useState([])  // {role:'user'|'assistant', content, rows?}
  ...
  const send = async (text) => {
    ...
    // Build history (text-only prior turns) for follow-up context.
    ...
    try {
      const res = await askCustomerAI(q, history.slice(0, -1))  // exclude the just-sent q (sent separately)
      ...
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: `Sorry — I couldn't answer that. ${err.message || ''}`.trim(),
        rows: [],
        isError: true,
      }])
    } finally {
  ...
  return (
    <div className="card p-0 overflow-hidden flex flex-col" style={{ height: 520 }}>
      {/* Header */}
      <div className="flex items-center gap-2.5 px-4 py-3 border-b border-gray-100 bg-gradient-to-r from-brand-50 to-white">
        ...
        <div>
          ...
          <p className="text-[11px] text-gray-500 mt-0.5">Natural-language questions over your customer &amp; order data</p>
        </div>
      </div>
      ...
      <div className="border-t border-gray-100 p-3">
        ...
        <div className="flex items-end gap-2">
          <textarea
            ...
            className="flex-1 resize-none text-sm border border-gray-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand-200 focus:border-brand-300 max-h-28"
            disabled={loading}
          />
          <button
            onClick={() => send()}
